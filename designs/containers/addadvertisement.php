		...
		<div class="col-md-8 col-md-offset-2">
			<form class="form-horizontal" action="" method="POST" enctype="multipart/form-data" autocomplete="off">
				...
				<div class="form-group">
					...
					<div class="col-sm-10">
						<?php $payedplans = getAllDataFromTable('userpayads',' where payed = 1 and adnumber > 0 and uid = '.$_SESSION['uid'],''); ?>
						<select class="form-control" name="plan" id="plan" required/>
							<?php for($j=0;$j<count($payedplans);$j++) { echo '<option value="'.$payedplans[$j]['id'].'|'.$payedplans[$j]['clicks'].'">'.$payedplans[$j]['title'].' (Ads #: '.$payedplans[$j]['adnumber'].' Clicks #: '.$payedplans[$j]['clicks'].' )'.'</option>'; } ?>
						</select>
					</div>
				</div>
				<div class="form-group">
