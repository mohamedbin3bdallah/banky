...
if(isset($_POST['settingssubmit']))
{
	...
	if($_POST['password'] == $_POST['cmfpassword'])
	{
		...
		if(update('users',$set,' where id = '.$_SESSION['uid'])) header('Location: page.php?c=adveraccount&message=m1');
		else header('Location: page.php?c=adveraccount&message=m2');
	}
	else header('Location: page.php?c=adveraccount&message=m6');
}
?>