<?php
include('libs/addadvertisement.php');
if(checkAddAdsByUser($_SESSION['uid']))
{
	$where = 'where uid = '.$_SESSION['uid'];
	$resultsPerPage = 0;
	if($resultsPerPage != 0)
	{
		...
		$noOfrows = getNoOfRowsFromTable('myadvertisements',$where);
	...
	if(isset($_POST['advertisementsubmit']))
	{
		...
		if($_POST['plan'] != '' && $_POST['category'] != '' && $_POST['title'] != '' && $_POST['description'] != '')
		{
			...
			if($_POST['fage'] > $_POST['tage']) header('Location: ?c=addadvertisement&message=m2');
			...
			//$users = getAllDataFromTable('users',' where advertiser = 0 and id <> '.$_SESSION['uid'].$where2,'');
			...
			if(!empty($_FILES))
			{
				...
				if($_POST['ad'])
				{
					...
					if($views['adid'])
					{
						if(update('userpayads',' set adnumber = adnumber-1 ',' where id = '.$_POST['plan']) && update('users',' set addads = addads-1 ',' where id = '.$_SESSION['uid']))
						{
							//for($i=0;$i<count($users);$i++) { $views['uid'] = $users[$i]['id']; insertRow('viewadvertisements',$views); }
							header('Location: ?c=addadvertisement&message=m1');
						}
						else header('Location: ?c=addadvertisement&message=m2');
					}
					else header('Location: ?c=addadvertisement&message=m2');
				}
				else header('Location: ?c=addadvertisement&message=m3');
			}
			...
			else header('Location: ?c=addadvertisement&message=m4');
			
			/*}
			else header('Location: ?c=addadvertisement&message=m18');*/
		}
		else header('Location: ?c=addadvertisement&message=m5');
	}
	
}
else header('Location: ?c=myadvertisements');
?>